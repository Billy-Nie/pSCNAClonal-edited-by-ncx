...
#       Version: 0.0.1
#    LastChange: 2018-06-22 13:51:57
#       History:
# =============================================================================
'''
...
class StripePool(object):
    ...
    def _aggregate(self, yDown, yUp, stripeNum, noiseStripeNum=2, byTag=False, plot=True):
        ...
        baselineCluster, _ = self.__get_baseline_from_stripe(self, clusters, ycV)
    ...
    def _decompose(self, cId, clusters, statusYcV, byTag=False, plot=True):
        ...
        mSIdx = np.intersect1d(ca, sa)  # Return the sorted, unique values that are in both of the input arrays
            ...
            if len(segL) < decomposeNumThresh:
                if not byTag:
                    ...
                    for tempTag in tempTags:
                        tagIdx = 0
                        if "BASELINE" == tempTag:
                            raise Exception("Got baseline in decomposion\
                                            step!")
                            continue
                        ...
                        subSegL, subSegIdxL = map(list.zip(*tempL))
                        tempStripe = Stripe()
                        tempStripe.name = "{0}_{1}_{2}".format(str(cId), tempTag, str(tagIdx))
                        tempStripe.init_segs(subSegL, subSegIdxL)
                        tempStripe.tag = tempTag
                        self.stripes.append(tempStripe)
                        tagIdx = tagIdx + 1
                ...
                if len(pT) == 0:
                    tempStripe = Stripe()
                    tempStripe.sid = "{0}".format(str(cId))
                    subSegL, subSegIdxL = map(list, zip(*segsIDL))
                    tempStripe.init_segs(subSegL, subSegIdxL)
                    self.stripes.append(tempStripe)
                    return
                """
        ...
        #  TODO: check out
        if False and byTag:  # TODO : always False ?
