...
from pSCNAClonal.preprocess.data.elements.segment import Segment
from pSCNAClonal.preprocess.data.elements.stripe import Stripe
from pSCNAClonal.preprocess.data.pools.stripePool import StripePool
from pSCNAClonal.preprocess.utils import (BEDnParser, BEDParser,
                                            chrom_idx_to_name,
                                            chrom_name_to_idx,
                                            get_APM_frac_MAXMIN,
...
from pSCNAClonal.preprocess.utils import remove_outliers
...
class SegmentPool:
    ...
    def load_seg_bam(self, nBam, tBam, bedName,containsReadNum = True):
    ...
    def _calc_baseline(self, maxCopyNumber, subcloneNum, isPreprocess=False):
        ...
        if rdRatioLog.shape[0] == 0: # 如果行数为0
        ...
        rdRatioLog = rdRatioLog.reshape(rdRatioLog.shape[0], 1) # 把这个行向量转置为列向量
        y = np.ones(rdRatioLog.shape)
        rdRatioLog = np.hstack((rdRatioLog, y)) # 把rdRatioLog和y连接起来,把每一个rdRatioLog的后面增加一个1
        clusters = hcluster.fclusterdata(rdRatioLog,
                                         thresh,
                                         criterion="distance")
        mccs = Counter(clusters).most_common(maxCopyNumber * subcloneNum) # return the most common maxCopyNumber * subcloneNum metrcis

        # 找到最小的rdrlog和它对应的cluster
        rdrMinLog = float('Inf')
        clusterMin = -1
    ...
    # 根据每一个segment的pairedCounts为该segment计算APMFrac
    ...
    # 使用segments里每一个segment的LOHFrac以及baseThred计算
    # 它对应的LOHStatus，并统计true或者false的总数
    ...
    # 使用每一个segment的APMFrac以及baseline计算每一个segment的APMStatus
    # 统计segment的APMStatus为true的个数
