...
class JointModelTrainer(ModelTrainer):
    ...
    def predict_by_stripe(self, j):
        ...
        if self.stripePool.segPool.segments[j].LOHStatus != 'NONE':
            ...
            ll_CNA_j = self.model_likelihood._ll_CNA_by_stripe(self.model_parameters, j)
...
class JointModelLikelihood(ModelLikelihood):
    ...
    def ll_by_seg(self, model_parameters, j):
        ...
        ll += self._ll_CNA_by_stripe(model_parameters, j)
        ll += self._ll_LOH_by_stripe(model_parameters, j)

        return ll
        
    def _ll_CNA_by_stripe(self, model_parameters, j):
        H = self.config_parameters.allele_config_num
        K = self.config_parameters.subclone_num
    ...
    def _ll_LOH_by_stripe(self, model_parameters, j):
        H = self.config_parameters.allele_config_num
        K = self.config_parameters.subclone_num
